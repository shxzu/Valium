package dev.sinxkyuna.valium.utils.rotation.manager;

import dev.sinxkyuna.valium.Client;
import dev.sinxkyuna.valium.event.bus.Listener;
import dev.sinxkyuna.valium.event.bus.annotations.EventLink;
import dev.sinxkyuna.valium.event.impl.input.EventMovementInput;
import dev.sinxkyuna.valium.event.impl.player.EventJump;
import dev.sinxkyuna.valium.event.impl.player.EventSilentRotation;
import dev.sinxkyuna.valium.event.impl.player.EventTickAI;
import dev.sinxkyuna.valium.event.impl.player.EventYawMoveFix;
import dev.sinxkyuna.valium.event.impl.render.EventRender3D;
import dev.sinxkyuna.valium.module.modules.movement.MoveFix;
...
public class RotationManager implements Utils {
    ...
    public final Listener<EventMovementInput> eventMovementInputListener = event -> {
        ...
        if (Client.INSTANCE.getModuleManager().getModule(Scaffold.class).isEnabled()) {
            ...
            if (Scaffold.telly.getValue() && PlayerUtil.inAirTicks() <= 5) {
                return;
            }
        }
    ...
    private boolean doSilentMoveFix() {
        if (!doMoveFix()) return false;
        Client.INSTANCE.getModuleManager().getModule(MoveFix.class);
        return MoveFix.silent.getValue();
    }
