...
import dev.sinxkyuna.valium.event.impl.player.EventSilentRotation;
...
import dev.sinxkyuna.valium.module.modules.player.scaffold.tower.ConstantMotionTower;
import dev.sinxkyuna.valium.module.modules.player.scaffold.tower.NormalTower;
import dev.sinxkyuna.valium.module.modules.player.scaffold.tower.VulcanTower;
...
import dev.sinxkyuna.valium.utils.math.MathUtils;
import dev.sinxkyuna.valium.utils.math.RandomUtil;
import dev.sinxkyuna.valium.utils.mc.*;
import dev.sinxkyuna.valium.utils.rotation.RotationUtils;
...
public class Scaffold extends Module {
    ...
    public static final ModeSetting sprintMode = new ModeSetting("Sprint Mode", "Legit", "Legit", "Normal", "NoPacket", "Watchdog Slow");
    ...
    public final Listener<EventTickPre> eventTickPreListener = event -> {
        ...
        if (keepY.getValue() && keepYMode.isMode("Watchdog") && MoveUtils.isMoving2() && blocksPlaced >= 1) {
            if (keepYLowHop.getValue() && Client.INSTANCE.getModuleManager().getModule(Disabler.class).isEnabled()) {
                Client.INSTANCE.getModuleManager().getModule(Disabler.class);
                if (Disabler.watchdogMotion.getValue() && Client.INSTANCE.getModuleManager().getModule(Disabler.class).canLowHop) {
    ...
    private void updateRots() {
        ...
        if (blockData != null) {
            ...
            if (rotMode.isMode("Back")) {
                rotations = new float[]{mc.player.getYaw() + 180, 78f};
            }
        }
    }
    ...
    private void doNormalSprint() {
        if (sprint.getValue()) {
            switch (sprintMode.getMode()) {
                case "Legit":
                    if(telly.getValue() && PlayerUtil.inAirTicks() <= 3) {
